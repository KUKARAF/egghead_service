use crate::{
    ai::{
        generate::{call_generate, call_generate_iterative, with_userscript_header},
...
async fn run_generation(state: &Arc<AppState>, task_id: &str) -> anyhow::Result<()> {
    ...
    let task = sqlx::query_as::<_, TaskData>(
        "SELECT t.user_id, t.device_token_id, t.tab_url, t.prompt, t.page_html, t.action_recording, t.files_json, u.email as user_email, t.script_id
         FROM tasks t
    ...
    if let Some(ref sid) = task.script_id {
        // Iterative refinement: load conversation history and refine existing script
        #[derive(sqlx::FromRow)]
        struct MsgRow {
            role: String,
            content: String,
        ...
        let all_msgs: Vec<MsgRow> = sqlx::query_as(
            "SELECT role, content FROM script_messages WHERE script_id = ? ORDER BY created_at ASC",
        )
        .bind(sid)
        .fetch_all(&state.pool)
        .await?;

        // History excludes the last message (the user msg just inserted by refine_script)
        let history: Vec<(String, String)> = all_msgs.iter()
            .take(all_msgs.len().saturating_sub(1))
            .map(|m| (m.role.clone(), m.content.clone()))
            .collect();

        let resp = call_generate_iterative(
            &client,
            &task.tab_url,
            &task.prompt,
            &task.page_html,
        ...
        sqlx::query(
            "UPDATE userscripts SET script_code = ?, name = ?, updated_at = datetime('now') WHERE id = ?",
        )
        .bind(&full_script)
        .bind(&resp.name)
        .bind(sid)
        ...
        sqlx::query(
            "INSERT INTO script_messages (id, script_id, role, content) VALUES (?, ?, 'assistant', ?)",
        )
        .bind(&msg_id)
        .bind(sid)
        .bind(&full_script)
        ...
        sqlx::query(
            "UPDATE tasks SET status = 'done', script_name = ?, script_code = ?, match_pattern = ?, updated_at = datetime('now') WHERE id = ?",
        )
        .bind(&resp.name)
        .bind(&full_script)
        ...
        tracing::info!(task_id = %task_id, script_name = %resp.name, "iterative refinement complete");
    } else {
        // Initial generation
        ...
        // Auto-save generated script into the user's library
        ...
        if let Err(e) = saved {
            ...
            // Link task to newly created script
            let _ = sqlx::query(
                "UPDATE tasks SET script_id = ?, updated_at = datetime('now') WHERE id = ?",
            )
            .bind(&userscript_id)
            .bind(task_id)
            .execute(&state.pool)
            ...
            // Seed initial conversation messages
            ...
            if let Some(ref device_token_id) = task.device_token_id {
